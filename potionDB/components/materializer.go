...
func InitializeMaterializer(replicaID uint16, commitCh chan TMCommitInfo) (mat *Materializer,
	loggers []Logger) {
...
func listenToTM(id int64, logger Logger, replicaID uint16, commitCh chan TMCommitInfo, mat *Materializer) {
...
func (part *partition) handleMatPermsTxn(args MatBCTxnPermissions) {
	...
	part.handleMatCommit(MatCommitArgs{TransactionId: txnID, CommitTimestamp: clk, CommitType: BC_COMMIT})
}
...
func (part *partition) handleMatRemoteTxn(args MatRemoteTxnArgs) {
	...
	args.ReplyChan <- tools.Pair[int64, []crdt.UpdateObjectParams]{First: part.id, Second: part.bufRemoteNuUpds.Copy().ToSlice()}
...
func (part *partition) applyRemoteUpds(args MatRemoteTxn) {
	...
	//fmt.Printf("[MAT%d]Applying remote txn with %d upds.\n", part.partID, len(args.Upds))
	var hashKey uint64
	var obj VersionManager
