...
func (ic InternalClient) CloseClient() {
	ic.tmChan <- TransactionManagerRequest{Args: TMConnLostArgs{}}
}
...
func simpleUpdExample(client InternalClient) {
	...
	objId := crdt.MakeKeyParams("myKey", proto.CRDTType_ORMAP, "myBucket")
...
func (ic InternalClient) CreateUpdBuf(size int) []crdt.UpdateObjectParams {
	if size <= 0 {
		return make([]crdt.UpdateObjectParams, 0, 10)
	}
...
func (ic InternalClient) AddUpdate(keyParams crdt.KeyParams, upd crdt.UpdateArguments, updBuf []crdt.UpdateObjectParams) []crdt.UpdateObjectParams {
	return append(updBuf, crdt.UpdateObjectParams{KeyParams: keyParams, UpdateArgs: upd})
}

func (ic InternalClient) CreateReadBuf(size int) []crdt.ReadObjectParams {
	if size <= 0 {
		return make([]crdt.ReadObjectParams, 0, 10)
	}
	return make([]crdt.ReadObjectParams, 0, size)
}

func (ic InternalClient) DoSingleRead(keyParams crdt.KeyParams, read crdt.ReadArguments) crdt.State {
	return ic.DoRead([]crdt.ReadObjectParams{{KeyParams: keyParams, ReadArgs: read}})[0]
}

func (ic InternalClient) DoRead(readBuf []crdt.ReadObjectParams) []crdt.State {
...
func (ic InternalClient) AddRead(keyParams crdt.KeyParams, read crdt.ReadArguments, readBuf []crdt.ReadObjectParams) []crdt.ReadObjectParams {
	return append(readBuf, crdt.ReadObjectParams{KeyParams: keyParams, ReadArgs: read})
}
