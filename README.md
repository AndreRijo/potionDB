...
Many different settings can be changed through configuration files - check [protoServer.go](src/main/protoServer.go), method _loadConfigs()_ for a list of settings.
...
Afterwards, go inside the `baseFolder` and clonse the following repositories:
...
This will build a docker image named _mypotiondb_ from the source code.
