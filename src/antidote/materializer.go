...
func (c prepClockHeap) Len() int { return len(c.entries) }

//We want the heap's Pop() to return the lowest element, so we use < on "less". The smallest element is on h[0].
func (c prepClockHeap) Less(i, j int) bool { return c.entries[j].clk.IsHigher(c.entries[i].clk) }
...
func (c prepClockHeap) Push(value interface{}) {
	...
	if convValue.pos == cap(c.entries) {
...
func (c commitHeap) Len() int { return len(c.entries) }
...
func (part *partition) canCommit(args MatCommitArgs) (canCommit bool) {
	...
	if args.CommitTimestamp.IsLower(lowestPrep.clk) {
		//TODO: May have to check with stableClk due to the existence of remote transactions. Will have to consider this.
		//What is in my mind: a commit may be issued with a remote transaction already in mind but somehow not yet known by this partition.
		//I have to consider if this really can happen.
		return true
	}
...
func (part *partition) applyUpdates(updates []*UpdateObjectParams, commitClk clocksi.Timestamp) (downstreamUpds []*UpdateObjectParams) {
...
func (part *partition) handleMatCommitedClk(args MatCommitedClkArgs) {
	args.ReplyChan <- TimestampPartIdPair{
		Timestamp: part.stableClk.Copy(), partID: part.id}
}
