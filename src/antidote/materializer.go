...
type partitionData struct {
	...
	//Probably need an initial size parameter to be set up here
	bucketDB              map[string][]uint64 //Contains references to each CRDT by its bucket. Used for creating snapshots
...
var (
	...
	initialCRDTBucketSize     int //Initial size for the list of CRDTs in the bucket map. TODO: from configs
...
func listenForTransactionManagerRequests(id uint64, logger Logger, replicaID int16, downstreamOpsCh chan TMDownstreamRemoteMsg, materializer *Materializer) {
	...
	partitionData := partitionData{
		...
		bucketDB:            make(map[string][]uint64),
...
func applyReadAndReply(readArgs *MatReadCommonArgs, readLatest bool, readTs clocksi.Timestamp, txnId TransactionId, partitionData *partitionData) {
	...
	if !hasKey {
		obj = initializeVersionManager(readArgs.KeyParams, partitionData, true)
...
func applyUpdates(updates []*UpdateObjectParams, commitTimestamp *clocksi.Timestamp, partitionData *partitionData) (downstreamUpds []*UpdateObjectParams) {
	...
	for _, upd := range updates {
		...
		obj, hasKey := partitionData.db[hashKey]
		if !hasKey {
			obj = initializeVersionManager(upd.KeyParams, partitionData, false)
		}
...
func handleMatRemoteTxn(request MaterializerRequest, partitionData *partitionData) {
	...
	//fmt.Println("[MAT]Attempting to apply remote txn with", len(remoteTxnArgs.Upds), "updates")
	if copyTs.IsLowerOrEqualExceptFor(partitionData.stableVersion, partitionData.replicaID, remoteTxnArgs.ReplicaID) {
		//fmt.Println("[MAT]Actually applying remote txn with", len(remoteTxnArgs.Upds), "updates")
		...
		for _, upd := range remoteTxnArgs.Upds {
			...
			/*if upd.Bucket == "INDEX" {
				fmt.Println("[MAT]Applying remote update for index (key, bkt)", upd.Key, upd.Bucket)
			} else {
				fmt.Println("[MAT]Applying remote update for", upd.Key, upd.Bucket)
			}*/
...
func handleMatGetSnapshot(request MaterializerRequest, partitionData *partitionData) {
	...
	if _, has := snapshotArgs.Buckets["*"]; has {
		fmt.Println("Obtaining all buckets")
		...
		*/
		var currCRDT crdt.CRDT
		for bkt := range buckets {
			fmt.Println("Obtaining bucket", bkt)
			bucketHashes := partitionData.bucketDB[bkt]
			for _, hashKey := range bucketHashes {
				currCRDT = partitionData.db[hashKey].GetLatestCRDT()
				snapshots[i] = crdt.CrdtToProtoCRDT(hashKey, currCRDT)
				i++
			}
		}
	}
...
func handleMatApplySnapshot(request MaterializerRequest, partitionData *partitionData) {
	...
	for _, protoCRDT := range protoCRDTs {
		...
		//TODO: Also store in bucketDB
	}
	partitionData.stableVersion = clk.Copy()
}

func initializeVersionManager(keyParams KeyParams, partitionData *partitionData, isRead bool) (newVM VersionManager) {
	//For now, all CRDTs use the same version manager
	newCrdt := initializeCrdt(keyParams.CrdtType, partitionData)
	tmpVM := (&InverseOpVM{}).Initialize(newCrdt)
	if !isRead {
		hashKey := getHash(getCombinedKey(keyParams))
		partitionData.db[hashKey] = &tmpVM
		bucketList, has := partitionData.bucketDB[keyParams.Bucket]
		if !has {
			bucketList = make([]uint64, initialCRDTBucketSize)
		}
		partitionData.bucketDB[keyParams.Bucket] = append(bucketList, hashKey)
	}
	return &tmpVM
}
