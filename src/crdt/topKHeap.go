	...
	if !hasEntry || remsVc.GetPos(op.ReplicaID) < op.Ts {
		...
		if hasId {
			...
			if op.TopKElement.isHigher(elem.TopKElement) {
				...
				effectValue = TopKReplaceEffect{newElem: op.TopKElement, oldElem: elem.TopKElement, oldTs: oldTs}
			...
			if len(crdt.elems) < crdt.maxElems {
				...
				effectValue = TopKReplaceEffect{newElem: op.TopKElement, oldElem: min, oldMin: min, oldTs: oldTs}
