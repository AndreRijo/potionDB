...
PotionDB's protobuf interface is compatible with AntidoteDB's interface, thus AntidoteDB clients implementing its protobuf interface can be used in PotionDB with no changes required.
...
Afterwards, go inside the folder *potionDB* and run
...
**Note**: this involves a fairly lengthly amount of steps.
...
After RabbitMQ is started, go inside the folder *potionDB/src/* and run:
...
This will initialize 5 instances of PotionDB, ready to be used by [PotionDB's TPC-H Client](https://github.com/AndreRijo/TPCH-Client) for benchmarking.
...
```
docker run -p 8087:8087 -p 5672:5672 --cap-add=NET_ADMIN -v /FULL_PATH_TO_FOLDER_WITH_CONFIGS/cluster/normal/R1:/go/bin/extern/ -e CONFIG=/go/bin/extern/ --name potiondb1 andrerj/potiondb
docker run -p 8087:8087 -p 5672:5672 --cap-add=NET_ADMIN -v /FULL_PATH_TO_FOLDER_WITH_CONFIGS/cluster/normal/R2:/go/bin/extern/ -e CONFIG=/go/bin/extern/ --name potiondb2 andrerj/potiondb
docker run -p 8087:8087 -p 5672:5672 --cap-add=NET_ADMIN -v /FULL_PATH_TO_FOLDER_WITH_CONFIGS/cluster/normal/R3:/go/bin/extern/ -e CONFIG=/go/bin/extern/ --name potiondb3 andrerj/potiondb
docker run -p 8087:8087 -p 5672:5672 --cap-add=NET_ADMIN -v /FULL_PATH_TO_FOLDER_WITH_CONFIGS/cluster/normal/R4:/go/bin/extern/ -e CONFIG=/go/bin/extern/ --name potiondb4 andrerj/potiondb
docker run -p 8087:8087 -p 5672:5672 --cap-add=NET_ADMIN -v /FULL_PATH_TO_FOLDER_WITH_CONFIGS/cluster/normal/R5:/go/bin/extern/ -e CONFIG=/go/bin/extern/ --name potiondb5 andrerj/potiondb
```
